...
export class CoursesService implements OnModuleInit {
  ...
  async findAll(userId?: string) {
    ...
    // Tự động nạp dữ liệu chuẩn 300 từ vựng nếu phát hiện CSDL chưa đủ 30 bài học (10 chủ đề x 3 cấp mức Easy, Medium, Hard)
    if (
      courses.length === 0 ||
      !courses[0]?.units ||
    ...
    // Tất cả các chủ đề đều được MỞ KHÓA (isUnlocked = true)
  ...
  async autoSeedAllTopics() {
    ...
    // TOPIC 4: Mua Sắm & Giá Cả
    await seedTopicWithLevels(
      '🛍️ Chủ đề 4: Mua Sắm & Giá Cả (Shopping & Prices)',
      '30 từ vựng giá cả, giảm giá và mua sắm',
      4,
      [
    ...
    // TOPIC 5: Sức Khỏe & Y Tế
    await seedTopicWithLevels(
      '🏥 Chủ đề 5: Sức Khỏe & Y Tế (Health & Medical)',
      '30 từ vựng bác sĩ, đơn thuốc và y tế',
      5,
      [
    ...
    // TOPIC 6: Công Nghệ & Mạng Xã Hội
    await seedTopicWithLevels(
      '💻 Chủ đề 6: Công Nghệ & Mạng Xã Hội (Technology & Social Media)',
      '30 từ vựng phần mềm, AI, mạng xã hội',
      6,
      [
    ...
    // TOPIC 7: Tài Chính & Ngân Hàng
    await seedTopicWithLevels(
      '🏦 Chủ đề 7: Tài Chính & Ngân Hàng (Finance & Banking)',
      '30 từ vựng tài chính, giao dịch, chứng khoán',
      7,
      [
    ...
    // TOPIC 8: Giải Trí & Truyền Thông
    await seedTopicWithLevels(
      '🎬 Chủ đề 8: Giải Trí & Truyền Thông (Media & Entertainment)',
      '30 từ vựng phim ảnh, âm nhạc, nghệ thuật',
      8,
      [
    ...
    // TOPIC 9: Thể Thao & Sức Khỏe
    await seedTopicWithLevels(
      '🏀 Chủ đề 9: Thể Thao & Thể Hình (Sports & Fitness)',
      '30 từ vựng thể thao, giải đấu, vận động viên',
      9,
      [
    ...
    // TOPIC 10: Môi Trường & Thiên Nhiên
    await seedTopicWithLevels(
      '🌍 Chủ đề 10: Môi Trường & Thiên Nhiên (Environment & Nature)',
      '30 từ vựng biến đổi khí hậu, sinh thái',
      10,
      [
  ...
  async getUnitTest(unitId: string) {
    ...
    if (!unit) {
      throw new NotFoundException('Chủ đề bài thi không tồn tại');
    }

    const allVocabs = unit.lessons.flatMap((l) => l.lessonVocabularies.map((lv) => lv.vocabulary));
    
    if (allVocabs.length === 0) {
      throw new NotFoundException('Chưa có từ vựng để tạo bài test');
    }
  ...
  async submitUnitTest(userId: string, unitId: string, score: number) {
    let unit = await this.prisma.unit.findUnique({
      where: { id: unitId },
    });
