...
exports.VoiceBattleService = void 0;
const common_1 = require("@nestjs/common");
const prisma_service_1 = require("../../prisma/prisma.service");
let VoiceBattleService = class VoiceBattleService {
    constructor(prisma) {
        this.prisma = prisma;
    ...
    getLeagueTier(trophies) {
        if (trophies >= 500)
            return { name: 'Kim Cương', icon: '💎', color: '#06B6D4' };
        if (trophies >= 250)
            return { name: 'Vàng', icon: '🥇', color: '#F59E0B' };
        if (trophies >= 100)
            return { name: 'Bạc', icon: '🥈', color: '#94A3B8' };
        return { name: 'Đồng', icon: '🥉', color: '#B45309' };
    }
    async createMatch(userId, mode = 'SPEED_RUN') {
    ...
    async submitSpeech(userId, roomId, textSpoken, mode = 'SPEED_RUN') {
        const room = await this.prisma.voiceBattleRoom.findUnique({ where: { id: roomId } });
        if (!room) {
            throw new common_1.NotFoundException('Phòng đấu không tồn tại');
        }
...
exports.VoiceBattleService = VoiceBattleService;
exports.VoiceBattleService = VoiceBattleService = __decorate([
    (0, common_1.Injectable)(),
    __metadata("design:paramtypes", [prisma_service_1.PrismaService])
], VoiceBattleService);
//# sourceMappingURL=voice-battle.service.js.map